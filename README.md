# Lam Nguyen

I like building things with computers (and building computers).

In high school, I taught myself BrightScript over a summer at Roku and ended up coding most of a TV app. I led programming for my FRC team, and I helped take my CyberPatriot team to a Platinum finish at the state round — the highest overall score of the year. These days I'm a computer science student at Cal Poly SLO.

[![website](https://img.shields.io/badge/website-lamsauced.pages.dev-0f766e?style=for-the-badge)](https://lamsauced.pages.dev)
[![email](https://img.shields.io/badge/email-lnguyen0606%40outlook.com-2563eb?style=for-the-badge)](mailto:lnguyen0606@outlook.com)
[![linkedin](https://img.shields.io/badge/linkedin-in-0a66c2?style=for-the-badge)](https://www.linkedin.com/in/lnguyen0606)

## Currently

- Studying **computer science** at Cal Poly San Luis Obispo — transferred from De Anza College
- **Assistant Scoutmaster**
- Bilingual — **English & Vietnamese**

## Toolbox

[![programming](https://img.shields.io/badge/programming-Java%20%C2%B7%20Python%20%C2%B7%20BrightScript%20%C2%B7%20Scenegraph-0f766e?style=flat-square)](https://github.com/LamSauced?tab=repositories)
[![building-with](https://img.shields.io/badge/building%20with-Astro%20%C2%B7%20TypeScript%20%C2%B7%20JavaScript%20%C2%B7%20HTML%2FCSS%20%C2%B7%20Git-3178c6?style=flat-square)](https://astro.build)
[![systems](https://img.shields.io/badge/systems%20%26%20security-Windows%20Server%20%C2%B7%20Network%20monitoring%20%C2%B7%20Malware%20remediation-f05032?style=flat-square)](https://www.linkedin.com/in/lnguyen0606)

## Selected work

- **[ClubHub](https://github.com/LamSauced/ClubHub)** — a Roku TV app built during a summer internship at Roku; self-taught BrightScript and Scenegraph, coded ~80% of it.
- **[2024-Code](https://github.com/2643/2024-Code)** — robot code for FRC 2024 "Crescendo", built as programming lead for Team 2643.
- **[PoRTS](https://github.com/LamSauced/PoRTS)** — waveform generation and analysis in Python.
- **[vidpipe](https://github.com/LamSauced/vidpipe)** — messing around with local AI inference and diffusion to generate videos.

## Recognition

- CyberPatriot State Round — **Platinum**, highest overall team score of the year
- **AP Scholar with Distinction**
- S4CA Product Showcase Award (×2) · MESA Award
- LA Hacks Presenter

---

## About this repo

This is the source for [lamsauced.pages.dev](https://lamsauced.pages.dev) — a static personal site built with [Astro](https://astro.build) and deployed on Cloudflare Pages. Because the repo name matches my username, this README doubles as my GitHub profile.

### Developing

```sh
pnpm install
pnpm dev     # local preview at http://localhost:4321
pnpm build   # production build to dist/
```

- All content lives in `src/data/*.ts` — edit the data, not the components.
- Themes live in `src/themes/` — add a `*.css` palette plus one entry in `src/themes/index.ts` to ship a new color scheme.
